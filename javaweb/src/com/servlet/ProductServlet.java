...
public class ProductServlet extends HttpServlet {
	...
	protected void saveProduct(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String realative_url = "/javaweb/imgs/"+imgs.getSubmittedFileName();
		
		Product product = new Product( pname , new BigDecimal(price) , desc , realative_url);
	
		ProductDao dao = new ProductDao();
		dao.saveProduct(product);
	
		String abs_url = request.getRealPath("imgs") + "/"+ imgs.getSubmittedFileName();
		FileUtils.copyToFile(imgs.getInputStream(), new File(abs_url));
